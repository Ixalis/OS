  ...
  */
  for(pgit = 0; pgit < req_pgnum; pgit++)
  {
    ...
    if(MEMPHY_get_freefp(caller->mram, &fpn) == 0)
   {
    ...
    int victimPageNum;
    int swapFramePageNum;

    if ((find_victim_page(caller->mm, &victimPageNum) == -1) 
    || (MEMPHY_get_freefp(caller->active_mswp, &swapFramePageNum) == -1)){
    ...
    uint32_t victimPte = caller->mm->pgd[victimPageNum];
    int victimFramePageNum=PAGING_FPN(victimPte);
    __swap_cp_page(caller->mram, victimFramePageNum, caller->active_mswp, swapFramePageNum);
      pte_set_swap(&caller->mm->pgd[victimPageNum], 0, swapFramePageNum);
      newfp_str->fpn=victimFramePageNum;
   } 
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
  ...
  vma1->vm_id=1;
  vma1->vm_start=PAGING_MAX_PGN*PAGING_PAGESZ;
...
int pgaddr(struct pcb_t *caller, int rgid){
  printf("Start: %lu\n", caller->mm->symrgtbl[rgid].rg_start);
  printf("End: %lu\n", caller->mm->symrgtbl[rgid].rg_end);
  printf("Vmaid: %u\n", caller->mm->symrgtbl[rgid].vmaid);
  return 0;
}
